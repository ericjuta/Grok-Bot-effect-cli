...
import {
  ...
  projectGatewayAuthStatus,
  sanitizeGatewayLocalToolPermissionEvent,
...
export class SandHost {
  ...
  private wireExtensionGatewayEvents(extensions: HostExtensionRegistry): void {
    ...
    optionalMethod(auth, "subscribeToRenewal")?.((event: unknown) => {
      const payload = projectGatewayAuthStatus(auth, event);
      if (payload.lastRenewal !== null) {
        this.emit({ channel: "auth-status", payload });
      }
    });
    optionalMethod(
      extensions.api("local-tool-permission"),
      "subscribe"
    )?.((event: unknown) => {
      const payload = sanitizeGatewayLocalToolPermissionEvent(event);
      if (payload !== null) {
        this.emit({ channel: "local-tool-permission", payload });
      }
    });
