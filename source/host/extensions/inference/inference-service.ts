...
import {
  SandTranscriptionManager,
  type TranscriptionPort,
} from "../../../shared/node/cursor-backend/cursor-transcribe.js";
...
export function createHostInference(options: HostInferenceOptions) {
  ...
  let transcription: TranscriptionPort | undefined;
  ...
  return {
    ...
    transcribeAudio(args: {
      ...
      transcription ??= new SandTranscriptionManager({
        getCursorAccessToken: (request) => auth.getAccessToken(request),
        getMachineId: async () => await auth.getMachineId(),
      });
      return transcription.transcribe(args);
    },
