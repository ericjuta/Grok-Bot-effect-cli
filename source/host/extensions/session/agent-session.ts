...
import { getAgentAvatar, getAgentAvatarPng, getAgentNotificationAvatar, getAgentProfileText, updateAgentProfile } from "./session-profile-files.js";
...
export class SandAgentSessionStore {
  ...
  getAgentDir(agentId: string): string { assertValidSandAgentId(agentId); return join(this.rootDir, agentId); }
  ...
  getAgentNotificationAvatar(agentId: string) { return getAgentNotificationAvatar(this, agentId); }
  ...
  async listAgentIds(): Promise<string[]> { try { return (await readdir(this.rootDir, { withFileTypes: true })).filter((entry) => { if (!entry.isDirectory()) return false; try { assertValidSandAgentId(entry.name); return true; } catch { return false; } }).map((entry) => entry.name).sort(); } catch (error) { if ((error as NodeJS.ErrnoException).code === "ENOENT") return []; throw error; } }
  ...
  setSessionNotificationsEnabled(agentId: string, enabled: boolean): void { this.setSessionNotifyOnUpdates(agentId, enabled); }
