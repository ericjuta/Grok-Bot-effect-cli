...
/** Exact reason forwarded by the public 0.30 server interrupt route. */
export const USER_INTERRUPT_REASON = "user_interrupt";
...
export class RunnerRegistry {
  ...
  interruptAgentRun(agentId: string): { readonly hadActiveRun: boolean } {
    const wasInFlight = this.tm.runLifecycle.runningAgentIds().has(agentId);
    const hadActiveGroupMemberRun =
      this.activeGroupMemberRunners
        .get(agentId)
        ?.interrupt(USER_INTERRUPT_REASON) ?? false;
    const hadActiveDirectRun =
      this.runners.get(agentId)?.interrupt(USER_INTERRUPT_REASON) ?? false;
    const hadActiveRun = hadActiveDirectRun || hadActiveGroupMemberRun;
    this.tm.telemetry.reportTurnInterrupt({
      conversationId: agentId,
