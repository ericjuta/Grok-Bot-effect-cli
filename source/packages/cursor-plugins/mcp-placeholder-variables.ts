...
export function inferMcpPlaceholderVariables(config: { mcpServers?: unknown } | null | undefined): { type: "object"; properties: Record<string, PlaceholderProperty> } | undefined {
  ...
  for (const name of [...placeholders.keys()].sort()) { const fallback = placeholders.get(name), secret = isSecretPluginVariableName(name); properties[name] = { type: "string", title: humanize(name), description: fallback === undefined || fallback.length === 0 ? `Referenced as \`\${${name}}\` in this plugin's MCP configuration.` : secret ? `Referenced as \`\${${name}}\` in this plugin's MCP configuration. A configured secret default is hidden.` : `Referenced as \`\${${name}}\` in this plugin's MCP configuration. Defaults to \`${fallback}\` when left blank.`, ...(secret ? { writeOnly: true } : {}) }; }
