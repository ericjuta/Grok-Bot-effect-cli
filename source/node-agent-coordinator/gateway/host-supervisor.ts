...
export class SandHostSupervisor {
  ...
  async ensureConnection(signal?: AbortSignal): Promise<GatewayConnection> {
    ...
    if (cached != null) {
      ...
      const healthHeaders = {
        ...(cached.headers ?? {}),
        ...(cached.token == null || cached.token.length === 0
          ? {}
          : { authorization: `${GATEWAY_AUTH_SCHEME} ${cached.token}` }),
      };
      if (await fetchHealth(this.options.timing, cached.baseUrl, healthHeaders, this.options.onReachability) != null) {
