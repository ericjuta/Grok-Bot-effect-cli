...
## Effect TypeScript CLI

This tree now includes a headless `grok-bot` CLI built on Effect and
`@effect/cli`. It talks to the authenticated Grok Bot host gateway rather than
starting or replacing the desktop process, and provides:

- stable JSON one-shot commands and the complete known gateway catalog;
- ergonomic agent, chat, provider, attachment, avatar, health, and diagnostics
  commands;
- reconnecting SSE event streams with explicit possible-gap notices;
- a bounded NDJSON RPC process with client-side request transport cancellation
  for custom TUI integrations; and
- an MCP stdio server designed for direct use from oh-my-pi.

The 188-name client union catalog includes all 147 audited public 0.30 gateway
methods, the removed 0.18 compatibility surface, and the reconstruction's
headless extensions. This is client reachability, not a claim that all 188
methods are implemented locally: live service negotiation marks what the
running host actually advertises. Destructive calls require `--yes`, MCP is
read-only by default, and gateway credentials are never emitted in connection
output. The reconstructed gateway requires its random bearer token even on
loopback and for health checks. MCP defaults its input/output frames to 40 MiB,
while NDJSON RPC fixes both at 40 MiB. Each uses a 128 MiB aggregate result
reservation, and every active request reserves at least the gateway's 64 MiB
JSON response cap before wire projection. The resulting active-request limit
is two instead of an unbounded result queue. For supported videos up to 200 MiB,
`attachment upload` negotiates a reconstructed-host raw stream; stock or older
hosts fall back to incremental base64 JSON and their request-body limits remain
authoritative.

```sh
npm ci
npm run cli:build
./dist/cli/grok-bot.mjs doctor
./dist/cli/grok-bot.mjs services
...
- Node.js 26.5 or newer within the Node 26 line (`>=26.5.0 <27`)
